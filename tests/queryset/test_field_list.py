...
class TestOnlyExcludeAll(MongoDBTestCase):
    ...
    async def test_gt_gte_lt_lte_ne_operator_with_list(self):
        ...
        qs = [doc async for doc in Family.objects(ages__gt=[1.0])]
        assert len(qs) == 1
        assert qs[0].ages == [1.0, 2.0]

        qs = [doc async for doc in Family.objects(ages__gt=[1.0, 1.99])]
        assert len(qs) == 1
        assert qs[0].ages == [1.0, 2.0]

        qs = [doc async for doc in Family.objects(ages__gt=[])]
        assert len(qs) == 1
        assert qs[0].ages == [1.0, 2.0]

        qs = [doc async for doc in Family.objects(ages__gte=[1.0, 2.0])]
        assert len(qs) == 1
        assert qs[0].ages == [1.0, 2.0]

        qs = [doc async for doc in Family.objects(ages__lt=[1.0])]
        assert len(qs) == 1
        assert qs[0].ages == []

        qs = [doc async for doc in Family.objects(ages__lte=[5.0])]
        assert len(qs) == 2

        qs = [doc async for doc in Family.objects(ages__ne=[5.0])]
        assert len(qs) == 2

        qs = [doc async for doc in Family.objects(ages__ne=[])]
        assert len(qs) == 1
        assert qs[0].ages == [1.0, 2.0]
