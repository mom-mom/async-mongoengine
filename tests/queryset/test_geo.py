...
class TestGeoQueries(MongoDBTestCase):
    ...
    async def test_near(self):
        ...
        if PYMONGO_VERSION < (4,):
            assert await events.count() == 3
        assert [doc async for doc in events] == [event1, event3, event2]
        ...
        if PYMONGO_VERSION < (4,):
            assert await events.count() == 3
        assert [doc async for doc in events] == [event3, event1, event2]
    ...
    async def test_within_distance(self):
        ...
        assert await events.count() == 2
        events = [doc async for doc in events]
        ...
        events = self.Event.objects(location__within_distance=point_and_distance)
        assert await events.count() == 1
        assert await events.get_item(0) == event2
        ...
        events = events.order_by("-date")
        assert await events.count() == 2
        assert await events.get_item(0) == event3
    ...
    async def test_within_box(self):
        ...
        assert await events.count() == 1
        assert (await events.get_item(0)).id == event2.id
    ...
    async def test_within_polygon(self):
        ...
        assert await events.count() == 1
        assert (await events.get_item(0)).id == event1.id
    ...
    async def test_spherical_geospatial_operators(self):
        ...
        await Point.drop_collection()

        # These points are one degree apart, which (according to Google Maps)
        # is about 110 km apart at this place on the Earth.
        north_point = await Point(location=[-122, 38]).save()  # Near Concord, CA
        south_point = await Point(location=[-122, 37]).save()  # Near Santa Cruz, CA
        ...
        assert [doc async for doc in points] == [south_point]
        ...
        if PYMONGO_VERSION < (4,):
            assert await points.count() == 2
        assert [doc async for doc in points] == [south_point, north_point]
    ...
    async def test_linestring(self):
        ...
        assert [doc async for doc in roads] == [road]
    ...
    async def test_aspymongo_with_only(self):
        ...
        assert (await qs.as_pymongo().get_item(0))["location"] == {
            "type": "Point",
    ...
    async def test_2dsphere_point_sets_correctly(self):
        ...
        await Location(loc=[1, 2]).save()
        loc = await Location.objects.as_pymongo().get_item(0)
        assert loc["loc"] == {"type": "Point", "coordinates": [1, 2]}

        await Location.objects.update(set__loc=[2, 1])
        loc = await Location.objects.as_pymongo().get_item(0)
    ...
    async def test_2dsphere_linestring_sets_correctly(self):
        ...
        await Location(line=[[1, 2], [2, 2]]).save()
        loc = await Location.objects.as_pymongo().get_item(0)
        assert loc["line"] == {"type": "LineString", "coordinates": [[1, 2], [2, 2]]}

        await Location.objects.update(set__line=[[2, 1], [1, 2]])
        loc = await Location.objects.as_pymongo().get_item(0)
    ...
    async def test_geojson_PolygonField(self):
        ...
        await Location(poly=[[[40, 5], [40, 6], [41, 6], [40, 5]]]).save()
        loc = await Location.objects.as_pymongo().get_item(0)
        assert loc["poly"] == {
            "type": "Polygon",
        ...
        await Location.objects.update(set__poly=[[[40, 4], [40, 6], [41, 6], [40, 4]]])
        loc = await Location.objects.as_pymongo().get_item(0)
        assert loc["poly"] == {
            "type": "Polygon",
