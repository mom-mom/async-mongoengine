...
class TestContextManagers(MongoDBTestCase):
    ...
    async def test_switch_collection_context_manager(self):
        ...
        await Group.drop_collection()  # drops in default

        async with switch_collection(Group, "group1") as Group:
            await Group.drop_collection()  # drops in group1

        await Group(name="hello - group").save()
        assert 1 == await Group.objects.count()

        async with switch_collection(Group, "group1") as Group:
            assert 0 == await Group.objects.count()

            await Group(name="hello - group1").save()
    ...
    async def test_no_sub_classes(self):
        ...
        with no_sub_classes(A):
            ...
            async for obj in A.objects:
                assert obj.__class__ == A

        with no_sub_classes(B):
            assert await B.objects.count() == 2

            async for obj in B.objects:
                assert obj.__class__ == B

        with no_sub_classes(C):
            assert await C.objects.count() == 1

            async for obj in C.objects:
    ...
    async def test_query_counter_alias(self):
        ...
        async with query_counter() as q:
            ...
            await A.objects.create(name="A")
            assert await q.get_count() == 1
            a = await A.objects.first()
            assert await q.get_count() == 2
            a.name = "Test A"
            await a.save()
            assert await q.get_count() == 3
            # querying the other db should'nt alter the counter
            await B.objects().first()
            assert await q.get_count() == 3

        async with query_counter(alias="testdb-1") as q:
            assert await q.get_count() == 0
            await B.objects.create(name="B")
            assert await q.get_count() == 1
            b = await B.objects.first()
            assert await q.get_count() == 2
            b.name = "Test B"
            await b.save()
    ...
    async def test_creating_a_document_within_a_transaction(self):
        ...
        async with run_in_transaction():
            a_doc = await A.objects.create(name="a")
            another_doc = await A(name="b").save()
            assert (await A.objects.get(id=a_doc.id)).name == "a"
    ...
    async def test_transaction_updates_across_databases(self):
        ...
        b_doc = await B.objects.create(name="b")

        async with run_in_transaction():
            await a_doc.update(name="a2")
            await b_doc.update(name="b2")
    ...
    async def test_collection_creation_via_upserts_across_databases_in_transaction(self):
        ...
        async with run_in_transaction():
            await a_doc.update(name="a3")
            async with switch_db(A, "test2"):
                await a_doc.update(name="a4", upsert=True)
                await b_doc.update(name="b3")
            await b_doc.update(name="b4")

        assert "a3" == (await A.objects.get(id=a_doc.id)).name
        assert "b4" == (await B.objects.get(id=b_doc.id)).name
        async with switch_db(A, "test2"):
    ...
    async def test_an_exception_raised_in_transactions_across_databases_rolls_back_updates(
        ...
        assert "b" == (await B.objects.get(id=b_doc.id)).name
        async with switch_db(A, "test2"):
    ...
    async def test_exception_in_child_of_a_nested_transaction_rolls_parent_back(self):
        ...
        async def run_tx():
            try:
                async with run_in_transaction():
                    await a_doc.update(name="trx-parent")
                    try:
                        async with run_in_transaction():
                            await b_doc.update(name="trx-child")
                            raise TestRollbackError()
                    except TestRollbackError as exc:
                        # at this stage, the parent transaction is still there
                        assert (await A.objects.get(id=a_doc.id)).name == "trx-parent"
                        raise exc
